...
from staging_order import (
    SHOPEE_ORDER_PARQUET, TIKTOK_ORDER_PARQUET, LAZADA_ORDER_PARQUET, TIKI_ORDER_PARQUET, WEBSITE_ORDER_PARQUET
)

# Định nghĩa asset đầu ra - chỉ thay đổi folder từ staging sang cleaned
SHOPEE_ORDER_CLEANED_PARQUET = Dataset("s3://minio/cleaned/shopee/orders/year={{logical_date.year}}/month={{logical_date.strftime('%m')}}/day={{logical_date.strftime('%d')}}/{{ds}}_data.parquet")
# TIKTOK_ORDER_CLEANED_PARQUET = Dataset("s3://minio/cleaned/tiktok/orders/year={{logical_date.year}}/month={{logical_date.strftime('%m')}}/day={{logical_date.strftime('%d')}}/{{ds}}_data.parquet")
# TIKI_ORDER_CLEANED_PARQUET = Dataset("s3://minio/cleaned/tiki/orders/year={{logical_date.year}}/month={{logical_date.strftime('%m')}}/day={{logical_date.strftime('%d')}}/{{ds}}_data.parquet")
# LAZADA_ORDER_CLEANED_PARQUET = Dataset("s3://minio/cleaned/lazada/orders/year={{logical_date.year}}/month={{logical_date.strftime('%m')}}/day={{logical_date.strftime('%d')}}/{{ds}}_data.parquet")
# WEBSITE_ORDER_CLEANED_PARQUET = Dataset("s3://minio/cleaned/website/orders/year={{logical_date.year}}/month={{logical_date.strftime('%m')}}/day={{logical_date.strftime('%d')}}/{{ds}}_data.parquet")
...
with DAG(
    ...
    default_args=default_args,
    description='Làm sạch dữ liệu người dùng trên MinIO bằng DuckDB và lưu lại vào MinIO',
    schedule=[LAZADA_ORDER_PARQUET],  # Đặt lịch chạy DAG khi có dữ liệu mới trong Dataset Lazada
    start_date=datetime(2023, 1, 1),
    catchup=False,
