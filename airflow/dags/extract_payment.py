...
def extract_payment(**context):
    """Extract data from payment table and push to XCom"""
    try:
        ...
        with engine.connect() as conn:
            ...
            # Push data vào XCom
            context['ti'].xcom_push(key='payment_df', value=data)
            logging.info(f"Extracted {len(data)} rows from payment table")

    except Exception as e:
        logging.error(f"Error extracting data from payment: {str(e)}")
        raise

def save_payment(**context):
    """Save data to parquet file in MinIO"""
    try:
        # Lấy dữ liệu từ XCom
        data = context['ti'].xcom_pull(key='payment_df', task_ids='extract_payment')
        ...
        # Tạo object name
        object_name = f"cleaned/erp/payment/year={logical_date.year}/month={logical_date.month:02d}/day={logical_date.day:02d}/{logical_date.strftime('%Y-%m-%d')}_data.parquet"
        ...
        raise
with DAG(
    dag_id='extract_payment',
    default_args=default_args,
    #schedule=None,
    catchup=True,
    tags=['extract', 'payment'],
    description='Extract toàn bộ bảng payment từ PostgreSQL',
    params={
    ...
    save_task = PythonOperator(
        task_id='save_payment',
        python_callable=save_payment,
    )
