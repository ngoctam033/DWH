...
def extract_geo_location(**context):
    """Extract data from geo_location table and push to XCom"""
    try:
        ...
        with engine.connect() as conn:
            ...
            # Push data vào XCom
            context['ti'].xcom_push(key='geo_location_df', value=data)
            logging.info(f"Extracted {len(data)} rows from geo_location table")

    except Exception as e:
        logging.error(f"Error extracting data from geo_location: {str(e)}")
        raise

def save_geo_location(**context):
    """Save data to parquet file in MinIO"""
    try:
        # Lấy dữ liệu từ XCom
        data = context['ti'].xcom_pull(key='geo_location_df', task_ids='extract_geo_location')
        ...
        # Tạo object name
        object_name = f"layer=cleaned/data_model=geo_location/channel=erp/year={logical_date.year}/month={logical_date.month:02d}/day={logical_date.day:02d}/{logical_date.strftime('%Y-%m-%d')}_data.parquet"
...
with DAG(
    ...
    extract_task = PythonOperator(
        task_id='extract_geo_location',
        python_callable=extract_geo_location,
    )

    save_task = PythonOperator(
        task_id='save_geo_location',
        python_callable=save_geo_location,
    )
